...
* **Hướng 1.2**: Nếu bài báo **đã có sẵn mã nguồn/thư viện/hệ thống** (do đó không cần tạo mã nguồn Proof of Concept nữa) thì học viên **cần có hệ thống riêng của mình** hoặc tìm được **một hệ thống mã nguồn mở** để áp dụng giải pháp vào, và đánh giá xem vấn đề có thực sự được giải quyết như bài báo đề cập hay không. Mã nguồn của hệ thống riêng của học viên sẽ phải công bố sau này. Nếu học viên có hệ thống riêng hoặc tìm được hệ thống mã nguồn mở phù hợp thì bài báo có thể dùng cho phần phân tích tiếp theo.
